...
| R-09 | **Rate limit bypass via distributed clients** — attacker sends parallel requests to exhaust API resources | Availability | Medium | Medium | open | In-memory sliding window 60 req/min per client IP; 429 + `Retry-After` response. **Rev 3 does not address this** — tracked separately. | `src/api/middleware/rate_limit.py:16-72` | Medium — in-memory limiter is not shared across API replicas (web-gateway HPA 2-5 pods); each pod has an independent counter | Backend |
